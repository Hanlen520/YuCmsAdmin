...
class UeditorController extends CExtController
{
	 ...
	 * @var array
	 */
	public $thumbSize = array('height' => '500', 'width' => '500');
    ...
    private function _upload($action)
	{
		...
		{
			$info['url'] = $this->imageHandle($info['url']);

			$fileInfo = pathinfo($info['url']);
	...
	protected function imageHandle($fullName)
	{
		...
		if($this->thumbnail)
		{
			...
			if (isset($_GET['thumbWidth']) && isset($_GET['thumbHeight']))
				$image->thumb($_GET['thumbWidth'], $_GET['thumbHeight']);
			else
				$image->thumb($this->thumbSize['width'], $this->thumbSize['height']);
