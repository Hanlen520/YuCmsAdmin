...
<div class="pd-20">
    ...
    <div class="row cl">
        ...
        <div class="formControls col-3">
            <?php echo !$actions ? '[已全部授权]' : '请选择以下方法';?>
        </div>
        <div class="col-3"> </div>
    </div>
    <?php if ($actions) :?>
        ...
        <div class="formControls col-3" style="background-color:#edfbf2;border:1px solid #c6e9dd;width:500px">
            <ul class="ul-list" style="padding-top: 0px;margin-left: 10px">
                <table border="0" width="100%">
                    <tr height="35">
                        <td width="15%" title="全选"><input type="checkbox" class="selectAllCheck" data-tag="0"> <b>[全]</b></td>
                        <td width="35%" style="font-size: 14px"><b>方法</b></td>
                        <td style="font-size: 14px"><b>描述</b></td>
                    </tr>
                    <?php foreach ($actions as $k => $v) :?>
                        <input type=hidden name="items[url][<?php echo $k;?>]" value="<?php echo $v['url'];?>">
                        <tr height="35">
                            <td><input  type = "checkbox" class="selectCheck" data-tag="1" name = "items[name][<?php echo $k;?>]" value ="<?php echo $v['itemId'];?>"></td>
                            <td style="font-size: 14px"><?php echo str_replace($this->delimeter, ' ', $v['itemId']);?></td>
                            <td><input type="text" class="input-text" name="items[description][<?php echo $k;?>]" value="<?php echo str_replace($this->delimeter, ' ', $v['itemId']);?>"></td>
                        </tr>
                    <?php endforeach;?>
                </table>
            </ul>
    ...
    <div class="row cl">
        ...
        <div class="col-8 col-offset-3">
            <input class="btn btn-primary radius" type="button" value="&nbsp;&nbsp;授权&nbsp;&nbsp;">
        </div>
        <?php endif;  ?>
    </div>
    <?php $this->endWidget(); ?>
...
<script>
    $(function(){
        ...
        $('.btn').click(function(){
            $.ajax({
                type:'POST',
                url: "<?php echo $this->createUrl('add', array('name' => $data['name']))?>",
                dataType:'json',
