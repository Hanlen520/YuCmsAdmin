 ...
 */
define("eagle/dropdownbox/1.0.0/dropdownbox-debug", [ "$-debug", "juicer-debug", "mousewheel-debug" ], function(require, exports, module) {
    var $ = require("$-debug"), juicer = require("juicer-debug"), mousewheel = require("mousewheel-debug");
    // 常量
    var DDB_BOX_ZINDEX = 100, DDB_BOX_FOCUS_CLASSNAME = "dropdownbox-focus", DDB_BOX_EXPAND_DOWN_CLASSNAME = "dropdownbox-expand-down", DDB_BOX_EXPAND_UP_CLASSNAME = "dropdownbox-expand-up", DDB_BOX_INDEX = 0, DDB_SELECT_INDEX_NAME = "data-ddb-tabindex", DDB_CAPTION_VALUE_NAME = "data-ddb-value", DDB_ITEM_HOVER_CLASSNAME = "current";
    // 默认配置属性
    var defaults = {
        className: "",
        // 下拉列表框自定义样式类名
        ...
        wheelScrolling: true,
        // 滚轮带动列表滚动
        autoUseSelecting: false,
        // 滚轮上下滚动 或 键盘上下键切换时是否自动使用选择的值
        dependOnSelecting: true,
        // 滚轮上下滚动 或 键盘上下键切换时取决于已选择的项，注意：[autoUseSelecting=true]
        updateLinkage: true
    };
    /**
    ...
    var dropDownBox = function(selector, options) {
        var options = $.extend(true, {}, defaults, options || {}), $selects = $(selector).filter("select");
        // 判断目标选择器jQuery对象长度
        if ($selects.length == 0) {
            return null;
        }
    ...
    dropDownBox_init.prototype = {
        ...
        _convertToBox: function($select) {
            ...
            tabIndex = $select.prop("tabIndex") || $select.data(DDB_SELECT_INDEX_NAME) || (DDB_BOX_INDEX += 1), templates = dropDownBox_init._templates, // 得到下拉列表框模板
            json = {
                // 得到下拉列表框数据模型
                ddbValue: $.trim($caption.attr("value")) || "",
                // ddb value
            ...
            }, $box = $(juicer(templates, json));
            // 通过模板数据转换为jQuery DropDownBox 对象
        ...
        _correlative: function() {
            ...
            // 下拉框控件绑定更新事件
            dom.select.on("ddb-update.ddb-oc", function(e) {
                // 更新联动数据
        ...
        _drawing: function() {
            var dom = this.dom, _dom = this._dom, width = Number(this.options.width || 0) || 0;
            if (!!width && width > 12) {
        ...
        _simulation: function() {
            var that = this, dom = this.dom, $caption = that._dom.getCaption.call(this), $list = that._dom.getList.call(this);
            // 下拉列表项鼠标点击、进入、离开事件
            $list.undelegate("li", ".ddb-ds").delegate("li", "click.ddb-ds", function(e) {
                that._selected($(this));
                that._pullUp();
                that._boxExtend();
                e.stopPropagation();
            ...
            // 下拉列表框获取焦点、点击事件
            dom.box.off(".ddb-os").on("focus.ddb-os", function() {
                that._pullUp();
                that._boxFocus();
                ...
                if ($list.is(":visible")) {
                    ...
                    var $window = $(window), _windowTopSpace = $window.scrollTop() + document.documentElement.clientHeight - $(this).offset().top, _listSpace = $list.outerHeight(true) + 1, _captionSpace = $caption.outerHeight(true), _windowBotSpace = $(this).offset().top - $window.scrollTop() - _listSpace, _listIsUpwards = _windowTopSpace < _listSpace && _windowBotSpace > 0, _listTop = _listIsUpwards ? -_listSpace : _captionSpace;
        ...
        _selected: function($item) {
            var $select = this.dom.select, $caption = this._dom.getCaption.call(this), $items = this._dom.getItems.call(this), _ddbValue = $.trim($item.attr("value")), _ddbText = $.trim($item.text()), _index = $items.index($item);
            if ($select.length > 0) {
        ...
        _movePrev: function($item) {
            var _dom = this._dom, _dependOnSelecting = this.options.dependOnSelecting, _autoUseSelecting = this.options.autoUseSelecting, $list = _dom.getList.call(this), $curItem = $item || (_dependOnSelecting && _autoUseSelecting ? _dom.getSelectingItem : _dom.getActiveItem).call(this), $prevItem = $curItem.prev("li"), isPeaked = $prevItem.length == 0, $selectItem = !isPeaked ? $prevItem : $curItem;
            if (!isPeaked) {
        ...
        _moveNext: function($item) {
            var _dom = this._dom, _dependOnSelecting = this.options.dependOnSelecting, _autoUseSelecting = this.options.autoUseSelecting, $list = _dom.getList.call(this), $curItem = $item || (_dependOnSelecting && _autoUseSelecting ? _dom.getSelectingItem : _dom.getActiveItem).call(this), $nextItem = $curItem.next("li"), isTail = $nextItem.length == 0, $selectItem = !isTail ? $nextItem : $curItem;
            if (!isTail) {
        ...
        _dom: {
            ...
            getActiveItem: function() {
                var $list = this._dom.getList.call(this), $activeItem = $("li." + DDB_ITEM_HOVER_CLASSNAME, $list), isExist = $activeItem.length > 0;
                if (!isExist) {
            ...
            getSelectingItem: function() {
                var $list = this._dom.getList.call(this), $caption = this._dom.getCaption.call(this), $SelectingItem = null, ddbValue = $.trim($caption.attr(DDB_CAPTION_VALUE_NAME)) || "";
                if (!!ddbValue) {
    ...
    (function($) {
        var fnExtends = {}, methods = [ "append", "prepend" ];
        // html=>append、appendTo=>append、prependTo=>prepend
