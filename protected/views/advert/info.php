				<ul class="nav">
					<?php foreach($nav as $key=>$val):?>
						<li><a  href="<?php  echo $key ==46 || $key ==47 ? $this->createUrl('index/about',array('cid'=>$key )): $this->createUrl('info',array('cid'=>$key ));?>"><?php echo $val;?></a></li>
					<?php endforeach;?>
					<li><a href="<?php  echo $this->createUrl('index/index')?>">泰策首页</a></li>
				</ul>
			</div>
		</div>
	</div>
